...
public class ballMovingSpecial : MonoBehaviour
{
        ...
        if (!gameEnd)
        {
            ...
            if (Input.GetKey(keySpace))
            {
                if (isfinished)
                {
                    ...
                    {
                        hitTop = false; hitBottom = true;
                        hitLeft = false; hitRight = true;
                    }
                    else
                    {
                        hitTop = true; hitBottom = false;
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.name == "player1Top" || collision.gameObject.name == "player1Bottom")
        {
            if(firstHit)
            {
                firstHit = false;
        ...
        if (collision.gameObject.name == "player2Top" || collision.gameObject.name == "player2Bottom")
        {
            if(firstHit)
            {
                firstHit = false;
